...
year = 2018 # specify the year of measurements that should be considered

sql_engine = sql.create_engine('postgresql:///metadb') # connect to the database...

start = 26 # the number of the row to start at (in millions)
count = start
end = 50 # number of the row to end at (in millions) -- note: if end - start = too big, process will be killed
file_num = 2 # measurements_(year)_(file_num).csv name specification
